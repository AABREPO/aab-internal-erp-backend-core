...
public class WeeklyPaymentExpenseController {
    ...
    @PutMapping("/{id}/remove-bill")
    public ResponseEntity<WeeklyPaymentExpense> removeBillCopy(@PathVariable Long id, @RequestBody String user) {
        WeeklyPaymentExpense updated = service.removeBillCopyUrl(id, user);
        return ResponseEntity.ok(updated);
    }
    @PutMapping("/{id}/restore-bill")
    public ResponseEntity<WeeklyPaymentExpense> restoreBill(@PathVariable Long id) {
        return ResponseEntity.ok(service.restoreBillCopyUrl(id));
    }
}