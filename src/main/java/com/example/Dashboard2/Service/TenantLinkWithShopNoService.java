...
public class TenantLinkWithShopNoService {
    ...
    public TenantLinkWithShopNo updateShopRentAndAdvance(Long tenantId, Long shopNoId, String monthlyRent, String advanceAmount){
        ...
        for (ShopNoWithTenantName shop : tenant.getShopNos()){
            // `shopNoId` in the API is the Shop No (shop_no_id), not the child row PK (`id`)
            if (shop.getShopNoId() != null && shop.getShopNoId().equals(shopNoId)){
                if (monthlyRent !=null){
        ...
        if (!shopFound){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Shop not found with ShopNoId: " + shopNoId);
        }
